...
void test_one_file_one_block(){
    ...
    if (write_chars(vfs, "a.txt", 0, 'a', 64)){
        if (write_chars(vfs, "a.txt", 1, 'b', 64)){
            if (read_chars(vfs, "a.txt", 0, 'a', 64)){
                if (read_chars(vfs, "a.txt", 1, 'b', 64))
                    ok = 1;
            }
        }
    }
...
void test_multiple_blocks(){
    ...
    if (write_chars(vfs, "a.txt", 0, 'a', 128)){
        if (read_chars(vfs, "a.txt", 0, 'a', 128))
            ok = 1;
    }
    isEqual(ok, 1, 1);
    vclosefs(vfs);    
